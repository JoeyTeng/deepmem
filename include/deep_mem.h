...
static inline bool block_is_allocated(block_head_t const *head) {
  return (*head) & A_FLAG_MASK;
}

static inline void block_set_A_flag(block_head_t *head, bool allocated) {
  *head = allocated ? (*head | A_FLAG_MASK) : (*head & (~A_FLAG_MASK));
}

static inline bool prev_block_is_allocated(block_head_t const *head) {
  return (*head) & P_FLAG_MASK;
}

static inline void block_set_P_flag(block_head_t *head, bool allocated) {
 ...
 **/
static inline block_size_t block_get_size(block_head_t const *head) {
 ...
 * Aligned size, including the block head
 **/
static inline void block_set_size(block_head_t *head, block_size_t size) {
  *head = (*head & (~BLOCK_SIZE_MASK)) /* preserve flags */
          | ALIGN_MEM_SIZE(size);      /* ensure rounds up */
}

static inline void *get_pointer_by_offset_in_bytes(void *p, int64_t offset) {
  return (uint8_t *)p + offset;
}

static inline int64_t get_offset_between_pointers_in_bytes(void *p, void *q) {
...
static inline int32_t get_offset_between_blocks(struct sorted_block *origin,
                                                struct sorted_block *target) {
  return get_offset_between_pointers_in_bytes((mem_t *)target, (mem_t *)origin);
}

static inline mem_size_t get_remainder_size(struct mem_pool const *pool) {
